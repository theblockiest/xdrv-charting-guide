...
<Tabs>
	<TabItem label="BG">
		...
		##### Expectations
		- Note quantizations should stick to 1/4ths, with 1/8ths, 12ths, or 16ths being used infrequently.
		- To aid reading, consecutive notes should not have large gaps between what lanes they are placed in (i.e 4-5 lane gaps).
		- Crossgears should not appear.
		- Much extra time should be given between inner lane and gear inputs on the same hand.
		...
		- Holds should be longer than 1 beat, and the releases of holds should be quantized to 4ths when possible.
		  - Hold-chords, if used, should be made symmetrical.
	...
	<TabItem label="NM">
		...
		- When chords are used, note density and complexity should be lower.
		  - Gearchords can be used as normal chords, except that note complexity should be evaluated.
		- Holds should be longer than 1/2 beat, and the releases of holds should be quantized to 4ths or 8ths when possible.
	...
	<TabItem label="Lower HY">
		...
		- Density no longer needs to be decreased when using gears, holds, and chords.
		- Hold notes can be any length, and releases can be quantized to any value.
	...
	<TabItem label="Upper HY/EX">
		...
		- In particular, crossgears can now be used with a higher note density and complexity.

		</Card>
	</TabItem>
</Tabs>

Although these rules can be helpful, bear in mind that intentional deviation from these rules, when given the right counterbalances, can result in charts that not only feel reasonable given the difficulty, but also test an underutilized skill within a controlled setting.
