...
Lua is a lightweight, high-level programming that is used to create XDRV modfiles. As a language, Lua is a great pick for XDRV modding for several reasons: it is easy to learn, requires minimal structuring, and is also familiar to many charters in the scene. Many other rhythm games, such as NotITG, use Lua to support modding functionality. Additionally, Trackmaker is built with Lua (specifically, it uses the LÖVE game engine).
...
If a parameter is a number, it must be some numerical representation.
...
In Lua, variable names can be composed of lowercase letters, uppercase letters, digits, and underscore, although the first character of a variable name cannot be a digit. Variable names are case-sensitive, meaning that `foo`, `Foo`, and `FOO` are all different variables. Good convention is to make your variable names descriptive of the value they store, using camelCase or underscores to separate words.
