...
<Tabs>
	...
	```lua
	xdrv.Set("camera_position_y",-0.1,"beat",0)
	xdrv.Set("camera_position_z",-0.1,"beat",0)
	xdrv.Set("camera_rotation_x",-6,"beat",0)
	```
	</TabItem>
	<TabItem label="Left">

	```lua
	xdrv.Set("camera_position_x",-0.5,"beat",0)
	```
	</TabItem>
	<TabItem label="Right">
...
Camera motion can also be good for per-beat effects. Consider popping the camera's y position up or swaying the camera's z rotation to each side.
...
<Aside type="tip" title="Visual Conventions Change">
